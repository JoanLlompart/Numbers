...
public class NumbersCat {
    ...
    public static String say(long n) {
        ...
        if (n >= 20 && n < 100) {
            ...
            if (n < 30) { //el minim ja no fa falta el especifiquem ja que esta fet en el primer if
                ...
                result = menysDe100(desena, dec, unicNums, uni);
                //return capitalLetter(desena[dec]) + "-" + unicNums[uni].toLowerCase();
            }
        }
        //Numeros del 100 al 999.
        if (n > 99 && n < 1000) { //Funcio menys de mil
            result = menysDeMil(n, desena, unicNums, uni);
        }

        //Menys de 1 milio.
        if (n > 999 && n < 1000000) {
            result = menysDeMilio(n, desena, unicNums, uni);
        }
    ...
    private static String menysDeMilio(long n, String[] desena, String[] unicNums, int uni) {
        String miler = "mil";
        int milena = (int) (n / 1000);//agafa la primera centena.
        int restaMil = ((int) n % 1000); //guarda el residu es a dir la resta de numeros.
        String resultMenysDeMilio = "";

        if (restaMil == 0) {
            resultMenysDeMilio = capitalLetter(miler);
        } else if (n > 1000 && n < 2000) {
            if (restaMil < 99) { // de 1001 a 1019.
                resultMenysDeMilio = capitalLetter(miler) + " " + menysDeMil(restaMil, desena, unicNums, uni);
            }

            if (restaMil > 19 && restaMil < 30) {
                resultMenysDeMilio = capitalLetter(miler) + " " + menysDeMil(restaMil, desena, unicNums, uni);
            }
    ...
    private static String menysDeMil(long n, String[] desena, String[] unicNums, int uni) {
        ...
        if (restaCent == 0) {
            ...
            if (unitatRestaCent == 0) { //Les decenas sense unitats.Ex : 20,30,40,50,etc
                resultCent = capitalLetter(centenas[cente]) + " " + desena[descenaRestaCent];
            }
            if (n > 100 && n < 120) {
            ...
            if (n > 130 && n < 200) {
                resultCent = capitalLetter(centenas[cente]) + " " + menysDe100(desena, descenaRestaCent, unicNums, uni).toLowerCase(); //Funciona fins a 109. A partir de alla falla.
            }
            if (n > 199 && n < 1000) { //Valors majors o iguals a 200 fins a 999.
                if (restaCent < 20) //Agafam la decena Restacent
                    ...
                    resultCent = capitalLetter(centenas[cente]) + " " + menysDe100(desena, descenaRestaCent, unicNums, uni).toLowerCase();
                }
            }
        }
        return resultCent;
